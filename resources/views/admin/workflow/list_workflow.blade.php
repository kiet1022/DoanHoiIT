...
<div class="container-fluid">
                        ...
                        <div class="col-md-12 col-sm-12 col-xs-12 px-0 d-flex">
                            <select name="activity" id="activity" class="form-control col-md-8 col-sm-8 col-xs-8">
                                ...
                                <option value="{{ $a->id }}" {!! changeSelectedStatus("$a->id", old('activity') ) !!} 
                                    @if(session('workflows')){!! changeSelectedStatus("$a->id", session('workflows')[0]->ofActivity->id."" ) !!} @endif >{{ $a->name }}</option>
                                @endforeach
                            </select>
    ...
    {{-- Hiển thị lại chi tiết công việc sau khi edit --}}
    @if(session('workflows'))
    @if(count(session('workflows')) > 0)
    <div class="row custom_panel">
        
        <div class="col-md-12 col-sm-12 col-xs-12 d-flex flex-wrap p-0" id="in-card-content">
            @foreach (session('workflows') as $workflow)
            <div class="col-4">
                <div class="card" style="height: 100%">
                    <div class="card-header">
                        <h5 class="card-title" style="color:red;">{{ $workflow->ofStudent->name }}</h5>
