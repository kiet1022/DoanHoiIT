    ...
    */
    public function postEditWorkFlowDetail(Request $req){
        ...
        try {
            ...
            $wfdetail->save();
            DB::commit();
            $this->data['workflows'] = WorkFlow::with(['details','ofStudent','ofActivity'])->where('activity_id',$wfdetail->ofActivity->id)->get();
            return redirect()->back()->with(config('constants.SUCCESS'),'Cập nhật công việc thành công')->with('workflows',$this->data['workflows']);
        } catch (Excetion $ex) {
            DB::rollback();
